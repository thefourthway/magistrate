...
_invalid_parser_data: list[tuple[str, typing.Type[Exception], typing.Callable[[Exception], bool]]] = [
    ...
    (
        ...
        lambda ex: typing.cast(BackwardsIncompatibilityViolation, ex).message == 'Migration declares itself backwards-incompatible, but defines a "down" section anyways'
    ),
    (
        'backwards_incompatible_multiple_declarations.mig.sql',
        BackwardsIncompatibilityViolation,
        lambda ex: typing.cast(BackwardsIncompatibilityViolation, ex).message == 'Migration cannot declare backwards-incompatibility statements more than once'
    ),
    (
        'backwards_incompatible_late_declaration.mig.sql',
        BackwardsIncompatibilityViolation,
        lambda ex: typing.cast(BackwardsIncompatibilityViolation, ex).message == 'Migration must declare itself backwards-incompatible before SQL statements are made'
    ),
    ...
    (
        ...
        MissingSection,
        lambda ex: typing.cast(MissingSection, ex).section == MigrationDirection.down
    ),
    (
        'section_missing_up_and_down.mig.sql',
        MissingSection,
        lambda ex: typing.cast(MissingSection, ex).section == MigrationDirection.up
    ),
    (
        'section_multiple_up_sections.mig.sql',
        DisjointedSections,
        lambda ex: typing.cast(DisjointedSections, ex).section == MigrationDirection.up
    ),
    (
    ...
    (
        ...
        lambda ex: typing.cast(InvalidMigrationVersion, ex).line == '-- ver: 2.5'
    ),
    (
